...
[![imap-iobroker CI](https://github.com/Lucky-ESA/imap-iobroker/actions/workflows/nodejs.yml/badge.svg)](https://github.com/Lucky-ESA/imap-iobroker/actions/workflows/nodejs.yml)


Note
====

This a fork of [Node-IMAP](https://github.com/mikebevz/node-imap) that aims to bring the package up-to-date.
...
imap-iobroker is an IMAP client module for [node.js](http://nodejs.org/).

This module does not perform any magic such as auto-decoding of messages/attachments or parsing of email addresses (imap-iobroker leaves all mail header values as-is).
...
```javascript
var Imap = require('imap-iobroker'),
...
* **isready**() - Emitted when a connection to the server has been made and authentication was successful.
...
* **search**(< _array_ >criteria, < _function_ >callback) - _(void)_ - Searches the currently open mailbox for messages using given criteria. `criteria` is a list describing what you want to find. For criteria types that require arguments, use an _array_ instead of just the string criteria type name (e.g. ['FROM', 'foo@bar.com']). Prefix criteria types with an "!" to negate.
  ...
  `criteria` examples:
    ...
    * All messages that have 'imap-iobroker' in the subject header: [ ['HEADER', 'SUBJECT', 'imap-iobroker'] ]
    * All messages that _do not_ have 'imap-iobroker' in the subject header: [ ['!HEADER', 'SUBJECT', 'imap-iobroker'] ]
