...
const utf7 = require("utf7.1").imap;
const MONTHS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
const Parser = require("./Parser").Parser;
const parseExpr = require("./Parser").parseExpr;
const parseHeader = require("./Parser").parseHeader;
const validateUIDList = require("./utils").validateUIDList;
const deepEqual = require("./utils")._deepEqual;
const escape = require("./utils").escape;
const buildSearchQuery = require("./utils").buildSearchQuery;
const MAX_INT = Number.MAX_SAFE_INTEGER;
const KEEPALIVE_INTERVAL = 10000;
const MAX_IDLE_WAIT = 300000;
...
class Connection extends EventEmitter {
    ...
    connect() {
        ...
        let parser;
        const tlsOptions = {};
        if (this._sock && this._sock.readable) {
            // Close existing connection first
            return this._sock.once("close", function() {
                self.connect();
            }).end();
        }
        ...
        parser.on("body", function (stream, info) {
            ...
            if (self._curReq && self._curReq.fetchCache && self._curReq.fetchCache.hasOwnProperty(info.seqno)) {
                msg = self._curReq.fetchCache[info.seqno];
            }
            if (msg === undefined) {
                // Ensure self._curReq is defined
                if (!self._curReq) {
                    self._curReq = {};
                }
                // Ensure self._curReq.fetchCache is an object
                if (!self._curReq.fetchCache) {
                    self._curReq.fetchCache = {};
                }
                // Ensure self._curReq.fetching is an array
                if (!Array.isArray(self._curReq.fetching)) {
                    self._curReq.fetching = [];
                }
                msg = self._curReq.fetchCache[info.seqno] = {
            ...
            const thisbody = info.which != null ? parseExpr(info.which.toUpperCase()) : "";
    ...
    append(data, options, cb) {
        ...
        if (options.date) {
            if (!util.types.isDate(options.date)) throw new Error("`date` is not a Date object");
            cmd += ' "';
    ...
    openBox(name, readOnly, cb) {
        ...
        this._enqueue(cmd, function (err) {
            if (err) {
                ...
                if (self._box !== undefined) {
                    if (self._box.name == undefined) self._box.name = "";
                    self._box.name = name;
                    cb(err, self._box);
                } else {
    ...
    _fetch(which, uids, options) {
        ...
        this.debug && this.debug("FETCH: " + JSON.stringify(fetching));
        this.debug && this.debug("CMD: " + cmd);
    ...
    _resUntagged(info) {
        ...
        if (type === "bye") this._sock.end();
            ...
            //if (this._curReq && this._curReq.fullcmd) {
                this.debug && this.debug("this._curReq.fullcmd: " + this._curReq.fullcmd);
            }
            this.debug && this.debug("infos: " + JSON.stringify(info));
            if (this._curReq && this._curReq.fullcmd != null && /^(?:UID )?FETCH/.test(this._curReq.fullcmd)) {
                this.debug && this.debug("this._curReq.fullcmd: " + this._curReq.fullcmd);
                this.debug &&
                    this.debug(
                        "this._curReq.fetchCache[info.num]: " + JSON.stringify(this._curReq.fetchCache[info.num]),
                    );
