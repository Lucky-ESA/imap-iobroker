...
function parseFetch(text, literals, seqno) {
    ...
    // Check if list[i] is a string before converting to lowercase
        key = (typeof list[i] === "string") ? list[i].toLowerCase() : null;

        if (key) {  // Only process if key is a string
            val = list[i + 1];
            if (key === "envelope")
            ...
            // eslint-disable-next-line no-cond-assign
                ...
                val = val ? Buffer.from(val) : Buffer.from(""+val);
            ...
            // Log or handle cases where list[i] is not a string
            console.error(`Expected string for key, got ${typeof list[i]}:`, list[i]);
        }
    }
    return attrs;
}
...
function parseExpr(o, literals, result, start, useBrackets) {
    ...
    for (let i = start, len = o?.str?.length || 0; i < len; ++i) {
        if (!inQuote) {
            if (isBody) {
                if (o.str[i] === ']') {
...
function decodeBytes(buf, encoding, offset, mlen, pendoffset, state, nextBuf) {
    ...
    if (jsencoding.encodingExists(encoding)) {
        ...
        if (!isPartial && nextBuf) {
            ...
            let lookahead;
            const lookaheadBuf = Buffer.alloc(buf.length + nextBuf.length); //new Buffer(buf.length + nextBuf.length);
